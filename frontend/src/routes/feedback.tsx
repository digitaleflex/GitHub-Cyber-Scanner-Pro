        ...
        {categories.map((cat, i) => (
          <a key={i} href={`${gh}?template=feature_request.md&labels=${cat.tag}`} target="_blank" rel="noopener noreferrer"
            ...
            <div className="flex items-center gap-2">
              <span style={{ color: 'var(--brand-text)' }}>{cat.icon}</span>
      ...
      <div className="surface rounded-2xl p-5 mb-6 text-center" style={{ border: '1px solid var(--border)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-center text-xs">
          {[
            ...
            <div key={i}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center mx-auto mb-1.5 text-sm font-bold" style={{ background: 'var(--bg-alt)', color: 'var(--brand-text)', border: '1px solid var(--border)' }}>{s.step}</div>
