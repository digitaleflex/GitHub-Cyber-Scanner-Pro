...
function ThreatIntelPage() {
  ...
  if (isLoading) return <div className="flex justify-center py-24"><div className="w-8 h-8 border-2 border-[var(--red)] border-t-transparent rounded-full animate-spin" /></div>
  ...
  return (
    <div className="max-w-4xl mx-auto py-4 sm:py-8 animate-fade">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <KpiCard icon={<AlertTriangle size={18} />} value={kev?.total?.toLocaleString() || '0'} label="CISA KEV actives" iconColor="var(--red)" />
        <KpiCard icon={<Bug size={18} />} value={platforms?.total_exploits?.toLocaleString() || '0'} label="Exploits publics" iconColor="var(--amber)" />
        <KpiCard icon={<Zap size={18} />} value={(epss?.top?.length || 0).toString()} label="Top EPSS" iconColor="var(--cyan)" />
        <KpiCard icon={<Shield size={18} />} value={(recent?.total || 0).toString()} label="Critiques 30j" iconColor="var(--amber)" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="surface p-4 sm:p-5 lg:col-span-2" style={{ border: '1px solid var(--border)' }}>
          <div className="flex items-center gap-2 mb-4">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'var(--red-light)' }}>
            ...
            {kev?.top?.slice(0, 8).map((c: any, i: number) => (
              <Link key={i} to="/cve/$id" params={{ id: c.cve_id }}
                ...
                <div className="flex items-center gap-2 mb-1">
                  <span className="mono font-semibold" style={{ color: 'var(--cyan)' }}>{c.cve_id}</span>
                  {c.cvss_score && <span className="text-xs font-medium" style={{ color: 'var(--red)' }}>CVSS {c.cvss_score}</span>}
                </div>
          ...
          <div className="space-y-1.5">
            {recent?.cves?.slice(0, 8).map((c: any, i: number) => (
              <Link key={i} to="/cve/$id" params={{ id: c.cve_id }}
                ...
                style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border-light)', textDecoration: 'none' }}>
                <span className="w-2 h-2 rounded-full shrink-0" style={{ background: c.severity === 'CRITICAL' ? 'var(--red)' : 'var(--amber)' }} />
                <span className="mono font-semibold shrink-0" style={{ color: 'var(--cyan)' }}>{c.cve_id}</span>
          ...
          <div className="space-y-1.5">
            {epss?.top?.slice(0, 8).map((e: any, i: number) => (
              <Link key={i} to="/cve/$id" params={{ id: e.cve_id }}
                ...
                <span className="mono font-semibold" style={{ color: 'var(--cyan)' }}>{e.cve_id}</span>
                <div className="ml-auto flex items-center gap-2 shrink-0">
                  <div className="h-1.5 w-20 rounded-full overflow-hidden" style={{ background: 'var(--border)' }}>
                    <div className="h-full rounded-full" style={{ width: `${Math.min(e.epss * 100, 100)}%`, background: 'var(--cyan)' }} />
                  </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-6">
        <div className="surface p-4 sm:p-5" style={{ border: '1px solid var(--border)' }}>
          <div className="flex items-center gap-2 mb-3">
            <Globe size={15} style={{ color: 'var(--amber)' }} />
            <h2 className="h3" style={{ color: 'var(--text)' }}>Plateformes ciblées</h2>
          </div>
          <div className="space-y-1.5">
            {platforms?.top_platforms?.map((p: any, i: number) => (
              <div key={i} className="flex items-center justify-between rounded-lg p-2.5" style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border-light)' }}>
