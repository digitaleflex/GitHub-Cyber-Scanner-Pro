...
import { Search, Star, Shield, BookOpen, Hash, ExternalLink, SlidersHorizontal } from 'lucide-react'
...
function SearchPage() {
  ...
  const { data: results } = useSearch(debounced.length >= 2 ? sp : { ...sp, q: '' })
  const hasFilters = !!(severity || verdict)

  return (
    <div className="max-w-4xl mx-auto py-4 sm:py-8 animate-fade">
      <h2 className="text-lg font-semibold text-white mb-4">Recherche avancee</h2>
      ...
      {debounced.length >= 2 && results ? (
        ...
        <div className="text-center py-16 glass-card rounded-2xl">
          ...
          <p className="text-slate-500 text-sm">Tapez au moins 2 caracteres pour chercher</p>
