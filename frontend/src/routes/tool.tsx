...
function TrustMeter({ score }: { score: number }) {
  ...
  const labelColor = score >= 70 ? '#166534' : score >= 40 ? 'var(--mission-text)' : 'var(--critical-text)'
  const labelBg = score >= 70 ? 'var(--success-light)' : score >= 40 ? 'var(--mission-light)' : 'var(--critical-light)'
  const labelBorder = score >= 70 ? 'var(--success)' : score >= 40 ? 'var(--mission)' : 'var(--critical)'
  const r = 28; const circ = 2 * Math.PI * r
  return (
    <div className="surface rounded-2xl p-4 sm:p-5" style={{ border: '1px solid var(--border)' }}>
      <p className="caption mb-2" style={{ color: 'var(--brand-text)' }}>Score de confiance</p>
...
function ToolDetail() {
  ...
  if (!tool || tool.error) return (
    <div className="text-center py-16">
      ...
      <Link to="/" className="text-xs mt-2 inline-block" style={{ color: 'var(--decision-text)' }}>← Retour</Link>
    </div>
  )

  const verdictStyle = () => {
    if (tool.security_verdict === 'Critique') return { bg: 'var(--critical-light)', text: 'var(--critical-text)', border: 'var(--critical)' }
    if (tool.security_verdict === 'Suspect') return { bg: 'var(--mission-light)', text: 'var(--mission-text)', border: 'var(--mission)' }
    ...
    <div className="max-w-4xl mx-auto py-4 sm:py-8 animate-fade">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-2 space-y-3 sm:space-y-4">
          <div className="surface rounded-2xl p-4 sm:p-6" style={{ border: '1px solid var(--border)' }}>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
              ...
              {tool.stars != null && <span className="text-xs px-2 py-1 rounded-lg flex items-center gap-1" style={{ background: 'var(--bg-alt)', border: '1px solid var(--border)', color: 'var(--mission)' }}><Star size={10} /> {tool.stars.toLocaleString()} stars</span>}
          ...
          {tool.similar?.length > 0 && (
            <div>
              <h2 className="h3 mb-3 flex items-center gap-2" style={{ color: 'var(--text)' }}>
                <TrendingUp size={14} style={{ color: 'var(--decision)' }} /> Outils similaires
              </h2>
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="surface rounded-2xl p-4 sm:p-5" style={{ border: '1px solid var(--border)' }}>
            <p className="caption mb-3" style={{ color: 'var(--brand-text)' }}>Statistiques</p>
          ...
          <a href={tool.html_url} target="_blank" rel="noopener"
            ...
            style={{ border: '1px solid var(--border)', color: 'var(--decision-text)', textDecoration: 'none' }}>
