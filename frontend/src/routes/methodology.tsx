...
function MethodologyPage() {
  const sections = [
    {
      icon: <Database size={16} style={{ color: 'var(--amber)' }} />,
    ...
    {
      icon: <Target size={16} style={{ color: 'var(--red)' }} />,
    ...
    {
      icon: <Shield size={16} style={{ color: 'var(--cyan)' }} />,
  ...
  return (
    <div className="max-w-[900px] mx-auto py-8 sm:py-16 animate-fade">
        ...
        {sections.map((s, i) => (
          <div key={i} className="surface rounded-2xl p-5" style={{ border: '1px solid var(--border)' }}>
            <div className="flex items-center gap-2 mb-2">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'var(--surface-elevated)' }}>{s.icon}</div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {s.badges.map((b, j) => (
                <span key={j} className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border-light)', color: 'var(--text-muted)' }}>{b}</span>
              ))}
            </div>
          </div>
        ))}
      ...
      <div className="surface rounded-2xl p-5 mt-8 text-center" style={{ border: '1px solid var(--border)' }}>
        <div className="flex items-center justify-center gap-2 mb-2"><Cpu size={18} style={{ color: 'var(--amber)' }} /><h2 className="h3" style={{ color: 'var(--text)' }}>Formule de score final</h2></div>
        <div className="inline-block rounded-xl p-4 text-sm font-mono" style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border)', color: 'var(--text)' }}>
