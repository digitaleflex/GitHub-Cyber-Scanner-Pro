...
function SourcesPage() {
  const categories = [
    {
      icon: <ShieldAlert size={16} style={{ color: 'var(--red)' }} />,
    ...
    {
      icon: <BarChart3 size={16} style={{ color: 'var(--amber)' }} />,
    ...
    {
      icon: <Server size={16} style={{ color: 'var(--cyan)' }} />,
        ...
        {categories.map((cat, ci) => (
          <div key={ci}>
            <div className="flex items-center gap-2 mb-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'var(--surface-elevated)' }}>{cat.icon}</div>
