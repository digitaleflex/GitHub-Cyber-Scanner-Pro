    ...
    <div className="max-w-[900px] mx-auto py-8 sm:py-16 animate-fade">
      <div className="text-center mb-10 sm:mb-16">
        <h1 className="h1 mb-4" style={{ color: 'var(--text)' }}>
          Transformer le chaos en <span style={{ color: 'var(--amber)' }}>décisions</span>
        </h1>
      ...
      {/* Mission */}
      <div className="surface rounded-2xl p-6 mb-8" style={{ border: '1px solid var(--border)' }}>
        <div className="flex items-center gap-2 mb-4"><Rocket size={18} style={{ color: 'var(--cyan)' }} /><h2 className="h2" style={{ color: 'var(--text)' }}>Notre mission</h2></div>
        <blockquote className="body-sm sm:text-base leading-relaxed italic pl-4 mb-4" style={{ color: 'var(--text-secondary)', borderLeft: '2px solid var(--cyan)' }}>
        ...
        <div className="flex items-center gap-2 mb-4"><Star size={18} style={{ color: 'var(--amber)' }} /><h2 className="h2" style={{ color: 'var(--text)' }}>Notre vision</h2></div>
        <p className="body-sm leading-relaxed pl-4" style={{ color: 'var(--text-secondary)', borderLeft: '2px solid var(--amber)' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
        {[
          { icon: <Search size={16} style={{ color: 'var(--amber)' }} />, title: 'Collecter', desc: '30+ sources: GitHub, NVD, CISA KEV, Exploit-DB, abuse.ch, OTX, OSV, GHSA, Shodan, VirusTotal...' },
          ...
          { icon: <GitBranch size={16} style={{ color: 'var(--amber)' }} />, title: 'Corréler', desc: "Knowledge Graph Neo4j. CVE ↔ Exploit ↔ Outil ↔ MITRE ATT&CK. Plus rien n'est isolé." },
          { icon: <Target size={16} style={{ color: 'var(--red)' }} />, title: 'Prioriser', desc: 'Pas un CVSS brut. Score intelligent: EPSS, KEV, exploits disponibles, verdict IA.' },
          ...
          <div key={i} className="surface rounded-xl p-4" style={{ border: '1px solid var(--border)' }}>
            <div className="flex items-center gap-2 mb-2">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'var(--surface-elevated)' }}>{p.icon}</div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-8">
        {[
          ...
          <div key={i} className="surface rounded-xl p-3 text-center" style={{ border: '1px solid var(--border)' }}>
            <div className="flex justify-center mb-1.5" style={{ color: 'var(--cyan)' }}>{p.icon}</div>
      ...
      <div className="surface rounded-2xl p-6 mb-8" style={{ border: '1px solid var(--border)' }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
          {[
            ...
            <div key={i} className="rounded-xl p-3" style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border-light)' }}>
              <div className="caption mb-1" style={{ color: 'var(--amber)' }}>{t.cat}</div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {['NVD API', 'CISA KEV', 'abuse.ch APIs', 'AlienVault OTX', 'OSV.dev', 'GitHub API', 'VirusTotal API', 'Shodan API', 'SecurityTrails API'].map((t, i) => (
            <span key={i} className="text-[10px] px-3 py-1 rounded-full" style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}>{t}</span>
          ))}
        </div>
      </div>
      ...
      <div className="surface rounded-2xl p-6 text-center" style={{ border: '1px solid var(--border)' }}>
        ...
        <p className="text-[10px] sm:text-xs text-muted mt-4">
          100% open source &middot; Auto-hébergeable &middot; Licence MIT &middot; <a href="https://cyberbook.eurin.tech" style={{ color: 'var(--cyan)' }}>cyberbook.eurin.tech</a>
        </p>
      </div>
    </div>
  )
