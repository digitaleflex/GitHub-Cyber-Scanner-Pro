...
- **Linux (systemd):** `service/neuralos.service` → `/etc/systemd/system/`, then `systemctl enable --now neuralos`.
- **macOS (launchd):** `service/ng.hyperspace.neuralos.plist` → `~/Library/LaunchAgents/`, then `launchctl load <plist>`.
