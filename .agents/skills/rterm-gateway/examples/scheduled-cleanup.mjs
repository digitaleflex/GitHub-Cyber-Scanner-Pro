#!/usr/bin/env node
/**
 * Example: create a cron scheduled task on the gateway (runs headless, no UI).
 *
 * Reads current settings, appends a scheduled task to automation.scheduledTasks,
 * and writes settings back. The RTerm scheduler executes it at the cron time.
 *
 * Run:  node scheduled-cleanup.mjs [ws://127.0.0.1:17888]
 */
import process from 'node:process';
import { createRequire } from 'node:module';
const require = createRequire(import.meta.url);
const WS = (() => { for (const c of ['ws', '/Users/olu/work/RTerm/node_modules/ws']) { try { return require(c); } catch {} } throw new Error('need ws'); })();

const URL = process.argv[2] || process.env.RTERM_GW_URL || 'ws://127.0.0.1:17888';
const ws = new WS(URL);
let seq = 0; const pending = new Map();
const rpc = (method, params = {}, t = 60000) => new Promise((res, rej) => {
  const id = `c${++seq}`; const timer = setTimeout(() => rej(new Error('timeout ' + method)), t);
  pending.set(id, { res, rej, timer }); ws.send(JSON.stringify({ id, method, params }));
});
ws.on('message', (raw) => { const m = JSON.parse(raw.toString());
  if (m.id && pending.has(m.id)) { const p = pending.get(m.id); clearTimeout(p.timer); pending.delete(m.id); m.error ? p.rej(new Error(m.error.message)) : p.res(m.result); } });

ws.on('open', async () => {
  try {
    const settings = await rpc('settings:get');
    const automation = settings.automation || {};
    const tasks = automation.scheduledTasks || [];
    const task = {
      id: `friday-cleanup-${Date.now().toString(36)}`,
      name: 'Friday Night Cleanup',
      cron: '0 0 * * 5',               // every Friday at 00:00
      enabled: true,
      // Run on every server in the "cleanup-targets" group; falls back to local shell if no scope.
      groupId: 'cleanup-targets',
      command:
        'find /var/app/cache -type f -mtime +30 -delete; ' +
        'journalctl --vacuum-time=7d; ' +
        'curl -sf localhost:8080/health | grep -q ok',
    };
    const next = { ...settings, automation: { ...automation, scheduledTasks: [...tasks, task] } };
    await rpc('settings:set', { settings: next });
    console.log(`Created scheduled task "${task.name}" (${task.cron}) -> id ${task.id}`);
    console.log('The RTerm scheduler will run it automatically; no UI needed.');
    process.exit(0);
  } catch (e) { console.error('error:', e.message); process.exit(2); }
});
ws.on('error', (e) => { console.error('connect:', e.message); process.exit(4); });
