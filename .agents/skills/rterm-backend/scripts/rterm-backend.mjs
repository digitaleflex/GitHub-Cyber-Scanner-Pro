#!/usr/bin/env node
/**
 * rterm-backend.mjs — cross-platform lifecycle manager for the RTerm backend.
 *
 ...
 * standalone `rterm-backend` (gybackend) daemon.
 *
 * Usage:
 *   node rterm-backend.mjs <command> [flags]
 *
 * Commands:
 *   doctor                 Check Node, npm pkg, data dir, port, gateway.
 *   install                npm install -g rterm-backend
 *   uninstall              stop + npm uninstall -g rterm-backend
...
function gybackendLaunch() {
  ...
  try {
    ...
    const script = path.join(root, 'rterm-backend', 'bin', 'gybackend.js')
...
async function install() {
  c.head('Installing rterm-backend globally')
  const npm = npmBin()
  const r = sh(npm, ['install', '-g', 'rterm-backend'])
...
async function uninstall() {
  await stop()
  c.head('Uninstalling rterm-backend')
  const npm = npmBin()
  sh(npm, ['uninstall', '-g', 'rterm-backend'])
  c.ok('uninstalled')
}

async function start() {
  c.head('Starting rterm-backend')
  const launch = gybackendLaunch()
  if (!launch) { c.err('gybackend not installed. Run: node rterm-backend.mjs install'); process.exit(1) }
  const busy = await portInUse(PORT)
  ...
  if (argv.daemon) {
    ...
    catch { c.warn(`gateway not answering yet — check: node rterm-backend.mjs logs`) }
  } else {
...
function installService() {
  ...
  if (IS_MAC) {
    const plist = path.join(HERE, '..', 'service', 'ng.hyperspace.rterm-backend.plist')
    console.log(`  1. cp "${plist}" ~/Library/LaunchAgents/`)
    console.log(`  2. edit GYBACKEND_DATA_DIR in the plist (currently a placeholder)`)
    console.log(`  3. launchctl load ~/Library/LaunchAgents/ng.hyperspace.rterm-backend.plist`)
    ...
    const unit = path.join(HERE, '..', 'service', 'rterm-backend.service')
    console.log(`  1. sudo cp "${unit}" /etc/systemd/system/`)
    console.log(`  2. sudo systemctl daemon-reload`)
    console.log(`  3. sudo systemctl enable --now rterm-backend`)
    console.log(`  gybackend resolves to: ${gyb}`)
  }
}
