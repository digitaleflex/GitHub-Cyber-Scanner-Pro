...
Custom commands are in addition to the built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`. [Learn more](tui.md#commands).
...
Use *!`command`* to inject [bash command](tui.md#bash-commands) output into your prompt.
...
Use the `agent` config to optionally specify which [agent](agents.md) should execute this command. If this is a [subagent](agents.md#subagents) the command will trigger a subagent invocation by default. To disable this behavior, set `subtask` to `false`.
...
Use the `subtask` boolean to force the command to trigger a [subagent](agents.md#subagents) invocation. This is useful if you want the command to not pollute your primary context and will **force** the agent to act as a subagent, even if `mode` is set to `primary` on the [agent](agents.md) configuration.
...
OpenCode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](tui.md#commands).
