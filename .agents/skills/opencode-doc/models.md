...
Here the full ID is `provider_id/model_id`. For example, if you're using [OpenCode Zen](zen.md), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.

If you've configured a [custom provider](providers.md#custom), the `provider_id` is key from the `provider` part of your config, and the `model_id` is the key from `provider.models`.
...
You can also configure these options for any agents that you are using. The agent config overrides any global options here. [Learn more](agents.md#additional).
