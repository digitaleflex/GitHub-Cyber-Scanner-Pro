...
> **Note:** The `write` tool is controlled by the `edit` permission, which covers all file modifications (`edit`, `write`, `patch`, `multiedit`).
...
> **Note:** The `patch` tool is controlled by the `edit` permission, which covers all file modifications (`edit`, `write`, `patch`, `multiedit`).
...
> **Note:** This tool is disabled for subagents by default, but you can enable it manually. [Learn more](agents.md#permissions)
