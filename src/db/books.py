...
def save_book(repo_id, title, url, category, lemmas_str=None, type_ressource=None):
    """Enregistre (ou met a jour) une ressource/livre extrait d'un README."""
    try:
        ...
        logging.error(f"Erreur save_book: {e}")
        return False


def get_books_to_verify(limit=50):
    """Liens a reverifier (jamais verifies ou plus vieux que 24h)."""
    try:
        conn = _conn.get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
